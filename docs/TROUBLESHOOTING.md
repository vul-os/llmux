...
- **Stream stalls behind a proxy.** llmux's streaming client has no *wall-clock* deadline (long generations are legal) — stalls usually come from an intermediate proxy buffering SSE. The Vulos OS proxy already sets `X-Accel-Buffering: no` and flushes per read; do the same in your own reverse proxy (disable response buffering for `/v1/chat/completions` and `/api/ai/chat`).
- **A stream ended with `llmux: streaming upstream stopped responding`.** That is one of the two **liveness bounds** added in 0.1.5, and it is not a wall-clock deadline. `stream_first_byte_timeout_seconds` (default 60) bounds the wait for the first chunk; `stream_idle_timeout_seconds` (default 120) bounds the gap between chunks and is re-armed on every chunk, so a generation that keeps producing is never truncated however long it runs. A proxy that buffers SSE hard enough can trip the idle bound — fix the buffering rather than raising the number. Set either to a negative value to disable it, `0` for the default. Tokens already served are metered.
...
- **Postgres surprises** — **for the sidecar** (`llmux serve`), DSN precedence is `VULOS_DATABASE_URL` > `DATABASE_URL` > `LLMUX_POSTGRES` > config `postgres`, so a stale `LLMUX_POSTGRES` can be silently outranked by a shared DSN. **In library mode this is different since 0.1.5**: the document wins over the environment, and `DATABASE_URL` / `VULOS_DATABASE_URL` are not read at all — only `LLMUX_POSTGRES` is. Either way tables live under the `llmux` schema (`LLMUX_POSTGRES_SCHEMA` to change). See [configuration](configuration.md#configuration-precedence-depends-on-who-is-asking).
...
**A forked worker looks healthy and then hangs on the first real request.** This is the previous entry wearing a disguise, and it is the one that gets shipped. Measured in real php-fpm and again after `os.fork()` in Python: a child that inherited a loaded library answers `models` in about 0.1 ms — it is served from memory and never enters the scheduler — and then never answers `chat` at all. **A health check that lists models is a false green for a broken child.** Probe with a real dispatch, or load the library after the fork.

**Signal-handling conflicts.** Loading the library replaces some of your host's signal handlers. Measured on a JVM host: five are replaced (`SIGSEGV`, `SIGBUS`, `SIGFPE`, `SIGPIPE`, `SIGURG`) and three more gain `SA_ONSTACK`, including `SIGUSR2`, which HotSpot uses to suspend threads. **`SIGPROF` is *not* touched** — the commonly-cited profiler hazard does not exist here, and JFR is unaffected; see [`sdks/java/signal-probe.sh`](https://github.com/vul-os/llmux/blob/main/sdks/java/signal-probe.sh), which measures it rather than assuming it. Go chains to a pre-existing handler in most cases, but "most" is the honest word. On the JVM the clean fix is `libjsig`, and it is a flag on the **java launch command** — a library cannot add one to a process that already started, which is why the Java and Kotlin packages default to the sidecar.

**A cancelled stream still costs money.** Stopping the consumer is not the same as stopping the stream. `llmux_stream` stops as soon as your callback returns non-zero, but a *buffered* async wrapper in your language can let the callback run ahead of the consumer: in one measured case a consumer that took 3 of 10 chunks still caused all 10 to be generated **and metered**. Each language package's README states its own measured callback count under early exit — read that number before you rely on cancellation to cap spend.
...
**A library host connects to a database at construction time.** `New` connects and migrates eagerly when `cfg.Postgres` is set — `CREATE SCHEMA`, `CREATE TABLE` — which is the one thing that makes construction non-inert. It can still be set by a variable rather than by you, but the set of variables that can do it narrowed in 0.1.5: in library mode only the namespaced **`LLMUX_POSTGRES`** is read, and a `postgres` value stated in your document now wins over it. The host's own `DATABASE_URL` and `VULOS_DATABASE_URL` are ignored — before 0.1.5 they were adopted, so merely loading `libllmux` inside a Rails or Django process created llmux's tables in that application's production database. Check the config you actually built, not the one you wrote. The connect is also bounded now (`postgres_connect_timeout_seconds`) rather than able to park the calling thread forever against a black-holed DSN.
