...
llmux is the sovereign LLM gateway: routing, fallbacks, per-key budgets,
caching and live cost across every provider, behind a default-deny sovereignty
gate that keeps inference on your box unless you explicitly opt out. It ships
two ways in: as a **Go library** (`core/gateway.New` — no HTTP, no port,
nothing started until you ask), or as the **single binary** this guide deploys,
which speaks the OpenAI HTTP API and adds an embedded admin console. Both run
the identical dispatch path. If you're writing Go and want the library instead
of a separate server process, skip to [Client examples → embed it
locally](client-examples.md#embed-it-locally-no-separate-server-to-run); this
guide covers the binary — building/deploying it, connecting providers
(including keeping inference on-box), pointing Vulos OS and its apps at it with
`LLMUX_URL`, and setting up authentication.
