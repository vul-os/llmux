...
```bash
curl -fsSLO https://raw.githubusercontent.com/vul-os/llmux/v0.1.7/scripts/verify.sh
bash verify.sh --tag v0.1.7 --attest llmux_0.1.7_linux_amd64.zip
```
...
a fake upstream, so it works offline with no provider keys.
...
llmux is pre-1.0 (`v0.1.7`) and under active development. Two things are
