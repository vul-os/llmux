 ...
 * and adapted for llmux's Node SDK (sdks/node): a plain-TS library, no
 * framework, running `strictTypeChecked` — the only repo in the fleet that
 * adopted the strict tier over recommendedTypeChecked.
 *
 ...
 *   - Do NOT grep eslint.config.mjs for "strictTypeChecked" — a config can
 *     name that preset and still resolve no type information.
...
// ============================================================================
// CONFIG — llmux Node SDK (sdks/node) specifics.
// ============================================================================
const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url))

const CONFIG = {
  // Root of the JS/TS project — where eslint.config.mjs, tsconfig.json and
  // package.json live, and the cwd ESLint runs from.
  webRoot: path.resolve(SCRIPT_DIR, '..'),

  // tsconfig.json's "include" is ["index.ts", "fixtures/**/*.ts",
  // "test/**/*.ts"] — the repo root itself is NOT generically included (only
  // the single file index.ts is named), so the probe fixture must land in
  // one of the two covered directories. "test" is used here; eslint.config's
  // `files` list covers "test/**/*.ts" too.
  fixtureDir: 'test',
  fixtureName: '__lint_config_gate_probe.ts',

  // llmux's repo root — this SDK is the only package.json in the tree with
  // a typescript dependency (verified 2026-08-05), but the sweep is allowed
  // to reach the whole repo so a stray range added anywhere else is caught.
  ...
  // (assertion C). Measured 2026-08-05: eslint.config.mjs, index.ts,
  // fixtures/fake-llmux.ts, test/sidecar.test.ts = 4, plus this script once
  // committed = 5. A floor of 5 (or even 4) is brittle here — this SDK is
  // small on purpose, and removing a single fixture file during a refactor
  // would trip the gate for no real reason. A floor of 1 catches nothing (a
  // glob reduced to matching only eslint.config.mjs itself would still
  // pass). 3 is the deliberate middle: it tolerates losing up to two of the
  // five current files, but a broken/typo'd glob — which in practice matches
  // 0 or 1 file, see the mutation test in this branch's commit history —
  // still fails it.
  coverageFloor: 3,
}
