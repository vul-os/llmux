#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded llmux release artifact, fail-closed.
#
...
#     name as a regex — every "." in "llmux_9.9.9_linux_amd64.zip" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.2.0 llmux_0.2.0_linux_amd64.zip
#   scripts/verify.sh --tag v0.2.0 --attest llmux_0.2.0_linux_amd64.zip
#   scripts/verify.sh --repo vul-os/llmux --tag v0.2.0 --out ~/Downloads ASSET
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/llmux-verify.XXXXXX")"
...
ART   = "llmux_9.9.9_linux_amd64.zip"
GOOD  = b"llmux-linux-amd64-bundle-bytes\n" * 64
...
trap_sums = (line(ART + ".sig", OTHER) +
             line("llmux_9X9X9_linux_amd64.zip", OTHER)).encode()
...
run_selftest() {
  ...
  local art="llmux_9.9.9_linux_amd64.zip"
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/llmux --base-url "$base/good" "$art"
